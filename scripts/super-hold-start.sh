...
# opens the omg-keys settings menu after a genuine 3-second hold. Paired with
...
# tap) -- see omg-keys/HANDOFF.md's "Gotchas" section. Implementing the timing ourselves
# with a plain press/release script pair sidesteps that entirely.

set -euo pipefail

PIDFILE="${XDG_RUNTIME_DIR:-/tmp}/omg-keys-super-hold.pid"
BIN="$(dirname "$(readlink -f "$0")")/../target/release/omg-keys"
